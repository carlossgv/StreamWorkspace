...
4. **Windows power call:** On clients that report Modern Standby (`AoAc`), StreamLight turns off the display through `SC_MONITORPOWER` so Windows can enter S0; it does not call `SetSuspendState` on that path. SleepStudy on a Dell G5 5590 showed the previous client `SetSuspendState(FALSE, FALSE, FALSE)` path entering hibernation (S4), while Windows Sleep entered S0 and woke by mouse. On clients without Modern Standby, StreamLight retains `SetSuspendState(FALSE, FALSE, FALSE)` with `SeShutdownPrivilege`, result checking, and failure logging. StreamTweak retains the existing host call pending separate hardware validation. Turning off the display requests the Modern Standby path but does not prove that Windows completed sleep. Session telemetry/cleanup follows the existing shutdown best-effort pattern before requesting host sleep; avoid claiming telemetry delivery is guaranteed when the machine suspends.
